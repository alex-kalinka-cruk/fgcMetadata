...
#' 
#' @return Returns an object of class `fgcMeta`. Also enters data into a project file.
#' @export
start_fgc_project <- function(data_dir, project_id, species, dir_entities = NULL){
  if(!dir.exists(data_dir)) dir.create(data_dir)
  tryCatch({
    proj <- rbind(metadata_tables$project,
                data.frame(title = project_id, species = species, stringsAsFactors = F))
